...
export default function HrLeaveManagement() {
  ...
  const handleSubmit = async (e) => {
   ...
   if (Number(form.noOfDays) < 1 || Number(form.noOfDays) > 366) {
  showNotification("Number of days must be between 1 and 366", "error");
  return;
}
...
if (!usernameRegex.test(lopForm.userName)) {
  showNotification("Username must be like vppl001 (4 letters + 3 digits)", "error");
  return;
}
    // Validate dates
